...
export async function GET() {
  try {
    ...
    const whereCotas = isAdmin ? { ano: currentYear } : { ano: currentYear, fracaoId: user?.fracaoId ?? '' };
    const wherePagamentos = isAdmin ? {} : { fracaoId: user?.fracaoId ?? '' };

    const wherePagamentosAno = isAdmin
      ? { dataPagamento: { gte: new Date(`${currentYear}-01-01`), lt: new Date(`${currentYear + 1}-01-01`) } }
      : { fracaoId: user?.fracaoId ?? '', dataPagamento: { gte: new Date(`${currentYear}-01-01`), lt: new Date(`${currentYear + 1}-01-01`) } };

    const [totalCotas, cotasPagas, cotasPendentes, pagamentos, allPagamentosAnoList, despesas, orcamento, fracoes, saldoTransitado, dividasPendentes, cotasExtrasPendentes] = await Promise.all([
      ...
      prisma.pagamento.findMany({ where: wherePagamentos, orderBy: { dataPagamento: 'desc' }, take: 10, include: { fracao: true } }),
      // Fetch all payments for the year with their linked cota/cotaExtra/divida (to know CC/FR split)
      prisma.pagamento.findMany({
        where: wherePagamentosAno,
        select: { valor: true, cotaId: true, cotaExtraId: true, dividaTransitadaId: true, dividaTransitada: { select: { tipo: true } } },
      }),
      isAdmin ? prisma.despesa.findMany({ where: { ano: currentYear }, orderBy: { dataEmissao: 'desc' }, take: 10 }) : Promise.resolve([]),
      ...
      isAdmin ? prisma.cotaExtraordinaria.aggregate({ where: { pago: false }, _sum: { valorTotal: true } }) : Promise.resolve(null),
    ]);
    ...
    // Dívidas transitadas pendentes do tipo OBRAS ("Outros") — contagem e soma
    // (usadas para surfacear um aviso dedicado no painel e decidir visibilidade do menu)
    ...
    // Split de receitas: 10% Fundo Reserva / 90% Conta Corrente para TODOS os
    // pagamentos relativos a cotas (regulares, extraordinárias e dívida transitada do tipo COTAS).
    // Exceção: dívida transitada do tipo OBRAS ("Outros") vai 100% para a Conta Corrente.
    let receitasCC = 0;
    let receitasFR = 0;
    for (const pag of (allPagamentosAnoList ?? [])) {
      const val = Number(pag?.valor ?? 0);
      if (pag?.dividaTransitadaId && pag?.dividaTransitada?.tipo === 'OBRAS') {
        // Dívida transitada "Outros": 100% para a conta corrente
        receitasCC += val;
      } else {
        // Todos os restantes pagamentos (cotas regulares, cotas extraordinárias,
        // e dívida transitada de cotas): 10% FR / 90% CC
        receitasFR += val * 0.10;
        receitasCC += val * 0.90;
      }
    }
    ...
    const totalDespesas = (despesas ?? []).filter((d: any) => d?.paga).reduce((acc: number, d: any) => acc + Number(d?.valor ?? 0), 0);
    const totalPendente = (totalCotas ?? []).filter((c: any) => c?.status !== 'PAGO').reduce((acc: number, c: any) => acc + Number(c?.valorTotal ?? 0), 0);

    // Totais GLOBAIS em euros de cotas pagas/pendentes (todas as dívidas, todos os anos)
    const whereGlobalCotas = isAdmin ? {} : { fracaoId: user?.fracaoId ?? '' };
    const [globalCotasPagasAggr, globalCotasPendentesAggr, globalDividasCotasPagasAggr, globalDividasCotasPendentesAggr] = await Promise.all([
      ...
      prisma.dividaTransitada.aggregate({ where: isAdmin ? { liquidada: true, tipo: 'COTAS' } : { liquidada: true, tipo: 'COTAS', fracaoId: user?.fracaoId ?? '' }, _sum: { valor: true } }),
      prisma.dividaTransitada.aggregate({ where: isAdmin ? { liquidada: false, tipo: 'COTAS' } : { liquidada: false, tipo: 'COTAS', fracaoId: user?.fracaoId ?? '' }, _sum: { valor: true } }),
    ]);
    const cotasPagasValor = Number((globalCotasPagasAggr as any)?._sum?.valorTotal ?? 0) + Number((globalDividasCotasPagasAggr as any)?._sum?.valor ?? 0);
    const cotasPendentesValor = Number((globalCotasPendentesAggr as any)?._sum?.valorTotal ?? 0) + Number((globalDividasCotasPendentesAggr as any)?._sum?.valor ?? 0);
    ...
    const saldoCC = saldoTransitadoCC + receitasCC - totalDespesas;
    ...
    const serialized = {
      ...
      totalCotasCount: totalCotas?.length ?? 0,
